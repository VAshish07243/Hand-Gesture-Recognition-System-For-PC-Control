...
while True:
    ...
    if len(lm_list) != 0:
        # Use the thumb (landmark ID 4) and index finger (landmark ID 8) tips
        ...
        # Define a threshold for a specific gesture (e.g., index finger and thumb close together)
        threshold = 30
        
        # If the index finger and thumb are close together, simulate a keyboard shortcut
        if distance < threshold:
            # Send a keyboard shortcut to switch tabs/windows
            pyautogui.hotkey('ctrl', 'tab')  # This hotkey switches to the next tab/window
            
            # Optional: Display a message or visual feedback for the gesture
            cv2.putText(img, "Switching Tabs/Windows", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
